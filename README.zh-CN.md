...
面向多智能体 AI 编程助手的统一技能管理系统。本工具包（以 Python 包形式分发）提供一套精选的可复用技能集合，以及用于在 Github Copilot、Claude Code、Cursor、Codex、Gemini CLI、Cline、OpenClaw、Hermes、CodeBuddy、Pi/DSH（DeepSeek Harness）等工具之间安装、配置和维护技能的集中式 CLI 命令。
...
<details>
<summary><b>设备自动化</b></summary>
<br>

**[Android Screen Reader](./mythril_agent_skills/skills/android-screen-reader/)**

通过 adb 读取已连接 Android 设备的当前屏幕。优先使用 UI 层级获取精确文本，再在需要时使用临时截图，并在分析完成后立即删除。
...
通过自然语言对话，为产品经理和业务分析师起草用户旅程地图和低保真线框图。生成自包含的工作区：`JOURNEY.md`（业务+mermaid 流程图）、`journey.json`（结构化数据）、`DESIGN.md`（视觉风格，Google 开源规范）与零依赖的 HTML/CSS/JS 预览（含 Map / Stage / Presenter 三视图）。不需要任何构建步骤——双击 `index.html` 即可在浏览器中打开。Session 中断后只需把工作区路径告诉 AI，它就能完整恢复上下文继续工作。

- **示例：** 画一份外卖小程序首单引导的用户旅程
- **依赖：** `git` CLI（可选，用于工作区版本历史）
