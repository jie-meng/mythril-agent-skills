...
在多仓库全栈工作区中做可行性调研和原型验证，不创建分支、不提交代码。临时改动保持未暂存状态。输出分析、调研发现和结论。如果可行，交给 `fullstack-impl` 正式实现。

- **示例：** 调研一下能不能迁移到 GraphQL
- **依赖：** 工作区需先通过 `fullstack-init` 初始化

**[Fullstack Impl](./mythril_agent_skills/skills/fullstack-impl/)**

在多仓库全栈工作区中实现功能、重构和修复。从 Jira/Confluence/GitHub/Figma 链接收集上下文，创建分支，委派给工作区 Agent，在文档仓库中跟踪进度。可以基于之前的调研结果实现。
...
| `fullstack-investigate` | 不提交代码的可行性调研 |
