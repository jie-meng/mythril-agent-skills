...
| [GH Operations](./mythril_agent_skills/skills/gh-operations/) | Use GitHub CLI (`gh`) for issue and pull-request workflows: read/write issues, inspect/create PRs, and add PR comments (including inline line comments). | "use gh", "gh issue", "gh pr", "在PR某行加comment", "行内评论", or "用gh评论PR" | `gh` CLI |
