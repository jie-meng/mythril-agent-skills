...
shared by the fullstack skill family (propose, apply). It replaces ad-hoc
LLM checks of "does fullstack.json exist", "does AGENTS.md exist", etc.
with a deterministic answer.

It also reports the github_repos flag (used by apply for PR creation) and
the docs_dir name (used to locate work directories), so a single invocation
gives the agent everything it needs to start.
