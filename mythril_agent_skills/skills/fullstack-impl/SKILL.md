...
This skill MUST NOT proceed unless it confirms the current working
directory is a valid fullstack workspace. Before any work, check for
ALL three markers:
...
docs directory config in fullstack.json) to function correctly.
...
### 1d. Scan existing work directories and route

List all existing work directories under `<docs-dir>/feat/`,
`<docs-dir>/refactor/`, and `<docs-dir>/fix/`. For each, peek at
`plan.md`'s `Status` line to classify it.

Then route the request based on what the user said. The full routing
logic lives in [`references/mode-selection.md`](references/mode-selection.md) —
**read it now**. The high-level routing:

| User signal | Status of matched dir | Route |
|-------------|----------------------|-------|
| Explicit follow-up verb ("follow up on X", "extend X", "在 X 基础上") | Closed | **Follow-up Mode** — see `followup-mode.md` |
| Explicit reading verb ("look at feat/X", "参考 feat/X", "based on X docs") | Any | **Reference Mode** — see `reference-mode.md` |
| Continue verb ("continue X", "继续 X") | Planning / In Progress | **Resume** the original Step 1-9 from where it stopped |
| Tweak / fix / log feedback verb | Done (PR not yet merged) | **Iteration Mode** — see `iteration-mode.md` |
| Scope overlaps a closed dir, no explicit verb | Closed | **Ask** the 3-option question (Follow-up / Reference / Independent) — see `mode-selection.md` |
| No matching dir, or clearly unrelated | — | **Fresh** standard Step 1-9 |
...
expensive (modifies a closed dir, creates phantom links).
...
**Do NOT read `fullstack.json` yourself or guess from repo URLs.**
Always run the bundled detection script — it gives a deterministic
answer:
...
- Exit code 1 → skip, workspace may not be initialized
...
This script reads that saved answer. Eliminates the failure mode where
GitHub Enterprise URLs like `git.company.com` get misclassified.
...
When this skill is invoked, scan existing work directories under
`<docs-dir>/feat/`, `<docs-dir>/refactor/`, and `<docs-dir>/fix/`
(this is part of Step 1 — Step 1d).

If the user references an existing work directory by name or feature:

1. Find the matching work directory.
2. Read all four documents (`analysis.md`, `plan.md`, `progress.md`,
...
5. Verify branches still exist in the affected repos.
6. If repos are already on the correct branch, skip checkout.
7. **Determine routing** — pick exactly one path based on the
   matched directory's `plan.md` Status and the user's request:

| Status | User's request | Route |
|--------|----------------|-------|
| `Planning` / `In Progress` | Continue / finish the work | **Resume** the original Step 1-9 from the last incomplete step |
| `Done` / `Complete` | Tweak / fix / log feedback (PR not merged yet) | **Iteration Mode** — see [`iteration-mode.md`](references/iteration-mode.md) |
| `Closed` | Explicit follow-up verb | **Follow-up Mode** — see [`followup-mode.md`](references/followup-mode.md) |
| `Closed` | Explicit reading verb | **Reference Mode** — see [`reference-mode.md`](references/reference-mode.md) |
| `Closed` | Neither verb (overlap only) | **Ask** the 3-option question — see [`mode-selection.md`](references/mode-selection.md) |
| Has `## Successors` table | User references the predecessor name | Read the LATEST successor first; resume / iterate / further follow-up against IT, not the original |

This handles all four real-world scenarios in one place:

1. A session was closed mid-work and the user starts a new session
   to continue → Resume.
2. The work was finalized but PR review is generating fixes → Iteration.
3. The work shipped last week and the user is reporting a production
   bug → Follow-up (`fix/<name>-v2/`) or fresh `fix/`.
4. The work shipped a month ago and the user wants to extend the
   feature → Follow-up (`feat/<name>-v2/`).
