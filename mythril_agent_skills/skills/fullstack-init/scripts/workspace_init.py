...
│   └── skills/        # 自定义技能（跨运行保留）
├── .claude            # → .agents（Claude Code 兼容软链接）
...
def _ensure_symlink(
    link_path: Path, target: str, report: dict[str, list[str]]
) -> None:
    """Create or fix a relative symlink. Replaces stale/wrong symlinks."""
    if link_path.is_symlink():
        if link_path.resolve() == (link_path.parent / target).resolve():
            return
        link_path.unlink()
    elif link_path.exists():
        return
    link_path.symlink_to(target)
    report["created"].append(f"{link_path.name} → {target} (Claude Code compat)")
...
def bootstrap_workspace(
    ...
    # --- Claude Code compatibility symlinks ---
    _ensure_symlink(root / "CLAUDE.md", "AGENTS.md", report)
    _ensure_symlink(root / ".claude", ".agents", report)
