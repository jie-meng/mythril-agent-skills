...
def discover_ignored_dirs(root: Path, docs_dir: str) -> list[str]:
    ...
    Everything except hidden dirs and WORKSPACE_TRACKED_DIRS is ignored.
    """
    ignored = []
    for entry in sorted(root.iterdir()):
        if not entry.is_dir():
...
def generate_gitignore(ignored_dirs: list[str]) -> str:
    """Generate .gitignore that ignores all non-workspace subdirectories."""
    lines = [
        "# =============================================================================",
